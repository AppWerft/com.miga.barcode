...
#define TAG "com.miga.barcode"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> TiBarcode_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("TiBarcode.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::TiBarcodeBindings::lookupGeneratedInit(
...
static void TiBarcode_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(TiBarcode_getBinding)->GetFunction());
}

static void TiBarcode_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::TiBarcodeBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry TiBarcodeBinding = {
	"com.miga.barcode",
	TiBarcode_init,
	TiBarcode_dispose
};
...
Java_com_miga_barcode_TiBarcodeBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.miga.barcode", &TiBarcodeBinding);
	titanium::KrollBindings::addExternalLookup(&(::TiBarcodeBindings::lookupGeneratedInit));
}
